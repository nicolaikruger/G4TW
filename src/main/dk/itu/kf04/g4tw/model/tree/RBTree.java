...
public class RBTree<Key extends ComparableByDimension, Value> {
    ...
    /**
     * Decrements the axis-value by one, and crops it inside a floor of 1 <= dimension <= MAX_DIMENSIONS.
     * @param axis  The current axis to increment.
     * @return The axis incremented or set to 1.
     */
    protected byte decrementAxis(byte axis) {
        axis -= 1;
        return (byte) axis > 0 ? axis : MAX_DIMENSIONS;
    }

    protected void decrementNodeAxis(RBNode node) {
        if (node == null) return;
        node.axis = decrementAxis(node.axis);
        decrementNodeAxis(node.left);
        decrementNodeAxis(node.right);
    }
    ...
    protected void incrementNodeAxis(RBNode node) {
        if (node == null) return;
        node.axis = incrementAxis(node.axis);
        incrementNodeAxis(node.left);
        incrementNodeAxis(node.right);
    }
    ...
    protected RBNode<Key, Value> rotateLeft(RBNode<Key, Value> node) {
        ...
        // Decrement node axis for the tmp BEFORE adding the node
        tmp.left = null;
        decrementNodeAxis(tmp);
        ...
        // Increment the node axis for the node
        incrementNodeAxis(node);
        return tmp;
    }
    ...
    protected RBNode<Key, Value> rotateRight(RBNode<Key, Value> node) {
        ...
        node.left = tmp.right;
        // Increment the tmp node axis - BEFORE adding it to node.
        tmp.right = null;
        incrementNodeAxis(tmp);
